  ...
  if (fontname)
    {
      ...
      if (!(pattern = FcNameParse ((FcChar8 *)fontname)))
        {
          fprintf (
              stderr, "error, cannot parse font name to pattern: '%s'\n",
              fontname
          );
...
void
drw_rect (
    Drw *drw, int x, int y, unsigned int w, unsigned int h, int filled,
    int invert
)
{
  if (!drw || !drw->scheme)
...
int
drw_text (
    Drw *drw, int x, int y, unsigned int w, unsigned int h, unsigned int lpad,
...
)
{
  ...
  if (!render)
  ...
  else
    {
      ...
      XFillRectangle (drw->dpy, drw->drawable, drw->gc, x, y, w, h);
      d = XftDrawCreate (
          drw->dpy, drw->drawable, DefaultVisual (drw->dpy, drw->screen),
          ...
          if (render)
            {
              ...
              XftDrawStringUtf8 (
                  d, &drw->scheme[invert ? ColBg : ColFg], usedfont->xfont, x,
                  ty, (XftChar8 *)utf8str, utf8strlen
              );
            }
          x += ew;
          w -= ew;
...
void
drw_font_getexts (
    Fnt *font, const char *text, unsigned int len, unsigned int *w,
    unsigned int *h
)
{
  XGlyphInfo ext;
