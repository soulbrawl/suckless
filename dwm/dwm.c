    ...
    for (k = start; k <= end; k++)
      for (i = 0; i < LENGTH (keys); i++)
        ...
        if (keys[i].keysym == syms[(k - start) * skip])
          for (j = 0; j < LENGTH (modifiers); j++)
            XGrabKey (
                dpy, k, keys[i].mod | modifiers[j], root, True, GrabModeAsync,
                GrabModeAsync
            );
    XFree (syms);
  }
...
manage (Window w, XWindowAttributes *wa)
{
  ...
  updatesizehints (c);
  updatewmhints (c);
  // added alwayscenter patch manually (two lines below) @2023-09-17_01-32
  ...
  XMoveResizeWindow (
      dpy, c->win, c->x + 2 * sw, c->y, c->w, c->h
...
setclientstate (Client *c, long state)
{
  ...
  XChangeProperty (
      dpy, c->win, wmatom[WMState], wmatom[WMState], 32, PropModeReplace,
  ...
  if (!c->neverfocus)
    {
      ...
      XChangeProperty (
          dpy, root, netatom[NetActiveWindow], XA_WINDOW, 32, PropModeReplace,
          (unsigned char *)&(c->win), 1
      );
    }
...
togglebar (const Arg *arg)
{
  ...
  updatebarpos (selmon);
  XMoveResizeWindow (
      dpy, selmon->barwin, selmon->wx, selmon->by, selmon->ww, bh
  );
  arrange (selmon);
}
